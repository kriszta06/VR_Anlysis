...
def extract_behavior_features(scenario_data):
    '''
    Extracts behavioral features from a 3D movement scenario.
    Calculates position, rotation, forward direction vectors and timestamps
    and creates statistical and dynamic features.
    The resulting feature vector will be used for clustering.
    Each row in 'scenario_data' represents one recorded frame.
    The matrix must contain exactly 10 columns with the following structure:
    Columns:
    0-2: HeadPosition(x,y,z) => 3D spatial position of the head in the scene.
    3-5: HeadRotation(Pitch, Yaw, Roll) => Head orientation angles in degrees.
    6-8: HeadForward(x,y,z) => Forward direction unit vector of the head.
    9: SceneTime => Timestamp of the frame in seconds.

    Parameters:

    :param scenario_data: numpy.ndarray => 2D array of shape (N,10) where N is the number of frames.

    Returns:
    numpy.ndarray => it is a 1D array. It contains 
    - spatial statistics, 
    - speed metrics, 
    - entropy measures, 
    ...
    total_distance = np.sum(movement_speeds)  # Recalculated for clarity
    ...
    pause_threshold = 0.01 + mean_speed if mean_speed > 0 else 1e-6
    ...
    if len(rot_delta_magnitudes) > window:
        ...
        head_stability_index = np.std(rot_delta_magnitudes) if len(rot_delta_magnitudes) > 1 else 0
