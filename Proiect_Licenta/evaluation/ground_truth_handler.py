...
def load_ground_truth(csv_path):
    """
    Loads clinical diagnoses from a CSV file and formats them into a dictionary.

    Parameters:
    csv_path (str): Path to the ground_truth.csv file.

    Returns:
    dict: A dictionary mapping 'Person_ID' to 'DIAGNOSIS' 
    """
...
def sync_data(program_results, ground_truth):
    """
    Synchronizes algorith output with clinical ground truth for comparison.

    Parameters:
    program_results (dict): Data loaded from behavioral_classification.json.
    ground_truth (dict): Data returned by load_ground_truth.

    Returns:
    tuple: (y_true, y_pred, person_ids) aligned lists for metric calculations.
    """
